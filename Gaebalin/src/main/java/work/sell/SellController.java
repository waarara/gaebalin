...
public class SellController {
	...
	public ModelAndView createSell(HttpServletRequest request){
		...
		if(fromCart != null){
			...
			codeParam.put("userCode", userCode);
			codeParam.put("productCode", productCode);

			String cartCode = request.getParameter("cartCode");
			String sellCode = sellService.retrieveSellCode(codeParam);
	...
	@RequestMapping(value="/work/sell/deleteSell.do", method=RequestMethod.GET)
	public ModelAndView deleteSell(HttpServletRequest request){
		...
		sellService.deleteSell(sellParam);

		mv.setViewName("redirect:/work/product/retrieveProductList.do");

		return mv;
	}
	
	@RequestMapping(value="/work/sell/ajaxBuyAll.do",method=RequestMethod.GET)
	@ResponseBody
	public Map<String, String> ajaxBuyAll(HttpServletRequest request
											, @RequestParam(value="counting", required=false) int counting
											, @RequestParam(value="cart", required=false) List<String> cartList
											, @RequestParam(value="code", required=false) List<String> codeList
											, @RequestParam(value="price", required=false) List<String> priceList
											, @RequestParam(value="count", required=false) List<String> countList){
		HttpSession session = request.getSession();
		
		Map<String, String> buyYnMap = new HashMap<String, String>();
		...
		System.out.println(codeList.getClass().getName());
		...
		*/
		...
		for(int i = 0; i < counting; i++) {
			...
			sellParam.put("productCode", codeList.get(i));
			sellParam.put("userCode", userCode);
			sellParam.put("sellPrice", priceList.get(i));
			sellParam.put("sellCount", countList.get(i));
			...
			codeParam.put("userCode", userCode);
			codeParam.put("productCode", codeList.get(i));

			String sellCode = sellService.retrieveSellCode(codeParam);

			cartParam.put("cartCode", cartList.get(i));
			sellParam.put("sellCode", sellCode);
			
			productParam.put("productCode", codeList.get(i));
			productParam.put("productCount", countList.get(i));
			
			cartService.deleteCart(cartParam);
			sellService.updateSellYn(sellParam);
			productService.updateProductCountMinus(productParam);
		};
		buyYnMap.put("buyYn", "success");
